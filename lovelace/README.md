...
3. **Check your card for a `template_url:` line and delete it.** Earlier
...
**If a card shows "Configuration error"**, two Home Assistant behaviours are
worth knowing, because between them they cost me a night on this card:

- Home Assistant renders only that generic string on the card face. The real
  message ("Custom element doesn't exist: …") appears **only** when you click
  the pencil to edit the dashboard.
- A failed module load is cached by the browser for the lifetime of the tab,
  and Home Assistant does not retry it. If you loaded the page while Home
  Assistant was restarting - or during the window at startup where the web
  server answers before custom integrations have registered their paths - the
  card stays broken until you load the page again with Home Assistant fully
  up, no matter what you fix in between.
