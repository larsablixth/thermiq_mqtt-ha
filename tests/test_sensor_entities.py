...
class TestHeatPumpSensor:
    ...
    @pytest.mark.xfail(
        reason="verifies the sensor/binary_sensor rework in #81; on current master these entities expose state via _state updated on events rather than reading _hpstate directly",
        strict=False,
    )
    ...
    def test_duration_sensor(self):
        ...
        sensor = self._make_sensor(
            hp,
            device_id="compressor_runtime_h",
            vp_reg="r17",
            vp_type="time",
            vp_unit="h",
        )
...
class TestHeatPumpBinarySensor:
    ...
    def _make_binary_sensor(
        self, heatpump, device_id="brine_pump_on", vp_reg="r10", bitmask=0x0001
    ):
    ...
    def test_is_on_specific_bit(self):
        ...
        sensor = self._make_binary_sensor(
            hp, device_id="boiler_6kw_on", vp_reg="r0d", bitmask=0x0002
        )
