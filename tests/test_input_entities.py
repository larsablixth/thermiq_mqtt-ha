...
# The native number/select/switch platforms arrive with the platform migration
# (#76). Until that lands these entities do not exist on master, so skip rather
# than fail - a red suite that is meant to be red gives no signal.
pytestmark_missing_platforms = pytest.mark.skipif(
    not (
    ...
    reason="native number/select/switch platforms not present (see #76)",
)


@pytestmark_missing_platforms
